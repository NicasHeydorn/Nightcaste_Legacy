...
def calcBonuses(attribute, fighter):
    ...
    for item in equipment:
        if len(item.bonus) > 0:
            if attribute in item.bonus:
                bonus += item.bonus[attribute]
    if attribute in fighter.bonus:
...
def sort_inventory():
    ...
    """
    gvar.game.player.inventory = sorted(
        gvar.game.player.inventory, key=lambda item: item.name)
...
def fov_distance_coef(spot):
    ...
    distance = get_distance(
        (spot[0], spot[1]), (gvar.game.player.x, gvar.game.player.y))
    player_fov_distance = int(gvar.game.player.fighter.skills[
                              "Awareness"] + gvar.game.player.fighter.perception)
